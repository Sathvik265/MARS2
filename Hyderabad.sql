-- ========================================================================
-- RESTAURANT BILLING SYSTEM - UNIFIED FIXED DATABASE SETUP SCRIPT (Hyderabad.sql)
...
-- 5. Updated track/shift names from ('`', '``', 'RBS1', 'RBS2') to ('`', '``', 'RBS', 'RBS1').
-- ========================================================================
...
BEGIN
    v_shift_type := COALESCE(p_shift_type, CASE
        ...
        WHEN CURRENT_TIME BETWEEN '18:00:00'::time without time zone AND '21:59:59'::time without time zone THEN 'RBS'::character varying
        ELSE 'RBS1'::character varying END);
...
BEGIN
    ...
    SELECT '``', 'SYS', CURRENT_DATE WHERE NOT EXISTS (SELECT 1 FROM public.sessions WHERE shift_name = '``' AND session_date = CURRENT_DATE);
    INSERT INTO public.sessions (shift_name, clerk_initials, session_date)
    SELECT 'RBS', 'SYS', CURRENT_DATE WHERE NOT EXISTS (SELECT 1 FROM public.sessions WHERE shift_name = 'RBS' AND session_date = CURRENT_DATE);
    INSERT INTO public.sessions (shift_name, clerk_initials, session_date)
