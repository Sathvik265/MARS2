...
export function generateAsciiReceipt(data, settings) {
  ...
  // \x1bx\x01: ESC x 1 (NLQ mode - Dark/High Density) + \x1bE: ESC E (Emphasized/Bold) + \x1bG: ESC G (Double-Strike - Max Ink Transfer)
  let ascii = "\x1bx\x01\x1bE\x1bG"; // ESC x 0 (Draft/High Speed) + ESC E (Emphasized/Bold)
  ...
  const headerHotelTitle = clerkInitials && clerkInitials !== "CLK" ? `${displayHotelName} (${clerkInitials})` : displayHotelName;
...
export function generateAsciiReport(title, columns, data, settings) {
  ...
  // \x1bx\x01: NLQ mode, \x1bE: Emphasized mode, \x1bG: Double-strike mode for maximum dark report printing
  let ascii = "\x1bx\x01\x1bE\x1bG";

  // Header Title
  const reportHotelTitle = clerkInitials && clerkInitials !== "CLK" ? `${hotelName} (${clerkInitials})` : hotelName;
